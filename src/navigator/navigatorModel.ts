...
export function favoriteNodes(entries: FavoriteEntry[], existing: Set<string>): NavigatorNode[] {
  if (entries.length === 0) return [{ type: 'message', id: 'favorites:empty', label: 'No favorite SKILL.md files yet.' }];
...
export function groupInstalledFiles(files: DiscoveredFile[]): NavigatorNode[] {
  if (files.length === 0) return [{ type: 'message', id: 'installed:empty', label: 'No supported local agent files found.' }];
  const byAgent = new Map<string, DiscoveredFile[]>();
  for (const file of files) {
    const agent = file.sourceId.split(':')[0];
    byAgent.set(agent, [...(byAgent.get(agent) ?? []), file]);
  }
  return [...byAgent.entries()].map(([agent, agentFiles]) => ({ type: 'agent', id: `agent:${agent}`, label: agentFiles[0]?.sourceLabel.split('/')[0] ?? agent }));
}

export function compareFiles(a: DiscoveredFile, b: DiscoveredFile): number {
  const ad = path.dirname(a.relativePath); const bd = path.dirname(b.relativePath);
  if (ad.toLowerCase() === bd.toLowerCase() && a.fileName !== b.fileName) return a.fileName === 'AGENTS.md' ? -1 : 1;
...
export function uriToFsPath(uri: string): string | undefined {
  try { const parsed = new URL(uri); return parsed.protocol === 'file:' ? decodeURIComponent(parsed.pathname) : undefined; } catch { return undefined; }
}
