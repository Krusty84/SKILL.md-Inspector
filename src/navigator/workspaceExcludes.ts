...
export function createWorkspaceExcludeMatcher(folder: vscode.WorkspaceFolder): WorkspaceExcludeMatcher {
  ...
  const patterns = Object.entries(excludes).filter(([, value]) => value === true).map(([pattern]) => pattern);
  return { excludes: (uri, name) => patterns.some((pattern) => matchesFilesExclude(folder.uri, uri, name, pattern)) };
}

export function matchesFilesExclude(root: vscode.Uri, uri: vscode.Uri, name: string, pattern: string): boolean {
  ...
  if (normalized.startsWith('**/')) return globPath(normalized.slice(3), relative) || globPath(normalized, relative);
...
function matchParts(pattern: string[], value: string[]): boolean {
  ...
  if (pattern[0] === '**') {
    return matchParts(pattern.slice(1), value) || (value.length > 0 && matchParts(pattern, value.slice(1)));
  }
  return value.length > 0 && globSegment(pattern[0] ?? '', value[0] ?? '') && matchParts(pattern.slice(1), value.slice(1));
}
...
function trimSlashes(value: string): string { return value.replace(/^\/+|\/+$/g, ''); }
