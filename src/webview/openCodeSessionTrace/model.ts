import type { NodeDetailsViewModel, SessionPathNode, TraceGraphViewModel } from '../../opencode/model';

export type WebviewToExtensionMessage =
  | { type: 'ready' }
  | { type: 'requestNodeDetails'; nodeId: string }
  | { type: 'requestPathNodeDetails'; pathNodeId: string }
...
export type ExtensionToWebviewMessage =
  ...
  | { type: 'pathNodeDetails'; pathNodeId: string; details: SessionPathNode }
...
export function isWebviewToExtensionMessage(value: unknown): value is WebviewToExtensionMessage {
  ...
  if (msg.type === 'requestPathNodeDetails') return typeof msg.pathNodeId === 'string' && msg.pathNodeId.length > 0 && msg.pathNodeId.length < 1000;
