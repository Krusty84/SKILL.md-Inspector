...
type FavoriteNode = { type: 'message'; label: string } | { type: 'favorite'; uri: string; exists: boolean; fsPath?: string };
...
export class FavoritesTreeProvider implements vscode.TreeDataProvider<FavoriteNode> {
  ...
  getChildren(node?: FavoriteNode): FavoriteNode[] {
    ...
    if (entries.length === 0) return [{ type: 'message', label: 'No favorite SKILL.md files yet.' }];
  ...
  getTreeItem(node: FavoriteNode): vscode.TreeItem {
    ...
    const item = new vscode.TreeItem(node.fsPath ? path.basename(path.dirname(node.fsPath)) : 'Missing Favorite', vscode.TreeItemCollapsibleState.None);
    ...
    if (node.exists && node.fsPath) {
      ...
      item.command = { command: 'skillMdInspector.openFavorite', title: 'Open Favorite', arguments: [node.uri] };
    }
    return item;
  }
