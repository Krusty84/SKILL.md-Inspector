...
export class WorkspaceTreeProvider implements vscode.TreeDataProvider<WorkspaceNode>, vscode.Disposable {
  ...
  getParent(node: WorkspaceNode): WorkspaceNode | undefined {
    ...
    return { type: 'workspaceDirectory', id: `workspace-directory:${parent.toString()}`, uri: parent, name, parentUri: parentUri(parent) };
  }

  dispose(): void { this.workspaceExplorer.dispose(); }
  onWorkspaceFoldersChanged(): void { this.workspaceExplorer.scheduleWorkspaceFoldersChanged(); }
  ...
  private isFavoriteUri(uri: vscode.Uri): boolean { return restoreFavorites(this.context.globalState.get(FAVORITES_KEY)).some((entry) => entry.uri === uri.toString()); }
}
