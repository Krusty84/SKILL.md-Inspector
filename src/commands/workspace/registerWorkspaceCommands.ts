  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      (target?: WorkspaceCommandTarget, selected?: WorkspaceExplorerNode[]) =>
        newEntry(d, 'file', target, selected),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.newFolder',
      (target?: WorkspaceCommandTarget, selected?: WorkspaceExplorerNode[]) =>
        newEntry(d, 'folder', target, selected),
    ),
    vscode.commands.registerCommand('skillMdInspector.workspace.selectSkillsFolder', () =>
      selectSkillsFolder(d.output),
    ),
    vscode.commands.registerCommand('skillMdInspector.workspace.addFolders', () =>
      addFolders(d.output),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.openFolderInNewWindow',
      (target?: WorkspaceCommandTarget) => openFolderInNewWindow(d, target),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.removeFolder',
      (target?: WorkspaceCommandTarget) => removeFolder(d, target),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.open',
      (target?: WorkspaceCommandTarget, selected?: WorkspaceExplorerNode[]) =>
        openFiles(d, false, target, selected),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.openToSide',
      (target?: WorkspaceCommandTarget, selected?: WorkspaceExplorerNode[]) =>
        openFiles(d, true, target, selected),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.rename',
      (target?: WorkspaceCommandTarget) => rename(d, target),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.delete',
      (target?: WorkspaceCommandTarget, selected?: WorkspaceExplorerNode[]) =>
        deleteTargets(d, target, selected),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.copy',
      (target?: WorkspaceCommandTarget, selected?: WorkspaceExplorerNode[]) =>
        setClipboard(d, 'copy', target, selected),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.cut',
      (target?: WorkspaceCommandTarget, selected?: WorkspaceExplorerNode[]) =>
        setClipboard(d, 'cut', target, selected),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.paste',
      (target?: WorkspaceCommandTarget) => paste(d, target),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.copyPath',
      (target?: WorkspaceCommandTarget, selected?: WorkspaceExplorerNode[]) =>
        copyPath(d, false, target, selected),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.copyRelativePath',
      (target?: WorkspaceCommandTarget, selected?: WorkspaceExplorerNode[]) =>
        copyPath(d, true, target, selected),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.openInTerminal',
      (target?: WorkspaceCommandTarget) => openTerminal(d, target),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.workspace.refreshFolder',
      (target?: WorkspaceCommandTarget) => refreshFolder(d, target),
    ),
  );
  context.subscriptions.push(
    d.view.onDidChangeSelection((event) => {
      const uri = event.selection.filter(isExplorerNode).find(isResourceNode)?.uri;
      void vscode.commands.executeCommand(
        'setContext',
        'skillMdInspector.workspaceSelectionWritable',
        uri ? vscode.workspace.fs.isWritableFileSystem(uri.scheme) !== false : false,
      );
    }),
  );
  void vscode.commands.executeCommand(
...
function resolveTargets(
  d: Deps,
  primary?: WorkspaceCommandTarget,
  selected?: readonly WorkspaceExplorerNode[],
  ...
  if (
    primaryNode &&
    isExplorerNode(primaryNode) &&
    isResourceNode(primaryNode) &&
    !nodes.some((node) => isResourceNode(node) && key(node.uri) === key(primaryNode.uri))
  )
...
async function resolveDestination(
  d: Deps,
  explicit?: WorkspaceExplorerNode,
  selection: readonly WorkspaceExplorerNode[] = [],
): Promise<vscode.Uri | undefined> {
  const explicitUri = destinationOf(explicit);
  if (explicitUri) return explicitUri;
  const selected = unique(selection)
    .map(destinationOf)
...
async function newEntry(
  ...
  selected?: WorkspaceExplorerNode[],
): Promise<void> {
  ...
  try {
    ...
    d.provider.invalidate(destination);
    d.provider.refreshNode(
      d.provider.getNodeForUri(destination) ?? d.provider.getRootForUri(destination) ?? targets[0],
    );
...
async function openFiles(
  ...
  for (const node of resolveTargets(d, target, selected).filter((n) => n.type === 'workspaceFile'))
...
async function rename(d: Deps, target?: WorkspaceCommandTarget): Promise<void> {
  ...
  try {
    ...
    d.provider.invalidate(node.parentUri);
    d.provider.refresh();
...
async function deleteTargets(
  ...
  const targets = pruneNested(
    resolveTargets(d, target, selected).filter(
      (node) => node.type !== 'workspaceRoot' && node.type !== 'workspaceError',
    ),
  );
...
async function setClipboard(
  ...
  clipboard = {
    operation,
    uris: pruneNested(resolveTargets(d, target, selected).filter(isResourceNode)).map(
      (node) => node.uri,
    ),
  };
  await vscode.commands.executeCommand(
...
async function paste(d: Deps, target?: WorkspaceCommandTarget): Promise<void> {
  ...
  for (const source of clipboard.uris) {
    ...
    try {
      if (clipboard.operation === 'copy')
        await vscode.workspace.fs.copy(source, targetUri, { overwrite });
      else await vscode.workspace.fs.rename(source, targetUri, { overwrite });
      d.provider.invalidate(dest);
  ...
  if (clipboard.operation === 'cut')
    clipboard = remaining.length ? { operation: 'cut', uris: remaining } : undefined;
  await vscode.commands.executeCommand(
    'setContext',
    'skillMdInspector.workspaceClipboardHasItems',
    !!clipboard?.uris.length,
  );
  d.provider.refresh();
}
...
async function copyPath(
  ...
  const text = resolveTargets(d, target, selected)
    .filter(isResourceNode)
    .map((node) =>
      relative
        ? vscode.workspace.asRelativePath(node.uri, true)
        : node.uri.scheme === 'file'
          ? node.uri.fsPath
          : node.uri.toString(),
    )
    .join('\n');
...
function refreshFolder(d: Deps, target?: WorkspaceCommandTarget): void {
  const node =
    target instanceof vscode.Uri
      ? (d.provider.getNodeForUri(target) ?? d.provider.getRootForUri(target))
      : target;
  if (node && (node.type === 'workspaceRoot' || node.type === 'workspaceDirectory')) {
    d.provider.invalidate(node.uri);
    d.provider.refreshNode(node);
  }
}
...
function isExplorerNode(value: unknown): value is WorkspaceExplorerNode {
  return (
    !!value &&
    typeof value === 'object' &&
    'type' in value &&
    String((value as { type: string }).type).startsWith('workspace')
  );
}
function isResourceNode(
  node: WorkspaceExplorerNode,
): node is Exclude<WorkspaceExplorerNode, { type: 'workspaceError' }> {
  return node.type !== 'workspaceError';
}
function unique(nodes: readonly WorkspaceExplorerNode[]): WorkspaceExplorerNode[] {
  const seen = new Set<string>();
  return nodes.filter((node) => {
    const k = isResourceNode(node) ? key(node.uri) : node.id;
...
function pruneNested<T extends { uri: vscode.Uri }>(nodes: readonly T[]): T[] {
  return nodes
    .filter((node, index) => nodes.findIndex((other) => key(other.uri) === key(node.uri)) === index)
    .filter(
      (node) =>
        !nodes.some(
          (other) => key(other.uri) !== key(node.uri) && isSameOrDescendant(node.uri, other.uri),
        ),
    );
}
function isSameOrDescendant(uri: vscode.Uri, ancestor: vscode.Uri): boolean {
  const u = key(uri);
  const a = key(ancestor);
  return u === a || u.startsWith(a.endsWith('/') ? a : `${a}/`);
}
...
async function pickUri(title: string, uris: vscode.Uri[]): Promise<vscode.Uri | undefined> {
  const picked = await vscode.window.showQuickPick(
    uris.map((uri) => ({ label: basename(uri), description: uri.toString(), uri })),
    { title },
  );
  return picked?.uri;
}
function report(output: vscode.OutputChannel, message: string, error: unknown): void {
