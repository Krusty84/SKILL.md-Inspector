...
import {
  ...
  type OpenCodeSessionCommandTarget,
} from './sessionUriResolver';
...
export function registerOpenCodeCommands(
  ...
  deps: {
    provider: OpenCodeSessionsTreeProvider;
  ...
  const load = async (target?: OpenCodeSessionCommandTarget) => {
    ...
    try {
      ...
      return {
        uri,
        session: await loadOpenCodeSession(uri, options.maxFileSizeBytes, {
          maxPreviewCharacters: options.maxPreviewCharacters,
        }),
      };
    } catch (e) {
      deps.output.appendLine(
        `OpenCode load failed for ${uri.toString()}: ${e instanceof Error ? (e.stack ?? e.message) : String(e)}`,
      );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('skillMdInspector.openCode.clearSessionsFolder', async () => {
      await deps.store.clear();
      await deps.provider.refresh();
    }),
    vscode.commands.registerCommand('skillMdInspector.openCode.refreshSessions', async () =>
      deps.provider.refresh(),
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.openCode.openReport',
      async (target?: OpenCodeSessionCommandTarget) => {
        const loaded = await load(target);
        if (loaded)
          OpenCodeSessionReportPanel.show(loaded.session, loaded.uri, context.extensionUri);
      },
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.openCode.openRawJson',
      ...
      async (target?: OpenCodeSessionCommandTarget) => {
        const summary =
          target && typeof target === 'object'
            ? (target as { summary?: { id?: string } }).summary
            : undefined;
        if (summary?.id) await vscode.env.clipboard.writeText(summary.id);
      },
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.openCode.copyFilePath',
      async (target?: OpenCodeSessionCommandTarget) => {
        ...
        if (uri) await vscode.env.clipboard.writeText(uri.fsPath || uri.toString());
      },
    ),
    vscode.commands.registerCommand(
      'skillMdInspector.openCode.revealInOS',
      async (target?: OpenCodeSessionCommandTarget) => {
        const uri = getOpenCodeSessionUriFromTarget(target);
        if (uri?.scheme === 'file') await vscode.commands.executeCommand('revealFileInOS', uri);
      },
    ),
  );
}
