...
export function registerOpenCodeCommands(context: vscode.ExtensionContext, deps: { provider: OpenCodeSessionsTreeProvider; store: OpenCodeSessionFolderStore; output: vscode.OutputChannel }): void {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('skillMdInspector.openCode.openTrace', async (target?: OpenCodeSessionCommandTarget) => { const loaded = await load(target); if (loaded) OpenCodeSessionTracePanel.show(context, loaded.session, loaded.uri, vscode.workspace.getConfiguration('skillMdInspector.openCode').get<boolean>('hideReasoningByDefault', true)); }),
