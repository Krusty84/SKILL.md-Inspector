...
export async function summarizeCandidate(uri: vscode.Uri, options: DiscoveryOptions, output?: vscode.OutputChannel): Promise<SessionSummary | undefined> {
  try {
    ...
    const normalized = normalizeSession(parsed.session, parsed.diagnostics, { maxPreviewCharacters: options.maxPreviewCharacters });
    return { uri, uriString: uri.toString(), fileName: basename(uri.path), size: stat.size, mtime: stat.mtime, id: normalized.id, parentId: normalized.parentId, title: normalized.title || basename(uri.path), updated: normalized.updated, model: normalized.model, provider: normalized.provider, toolCalls: normalized.metrics.toolCallCount, skillCalls: normalized.metrics.skillCallCount, errors: normalized.metrics.errorCount, children: [] };
...
export function buildSessionTree(items: SessionSummary[]): SessionSummary[] {
  ...
  for (const item of clones) {
    ...
    if (parent && parent !== item && parent.id !== item.id && (!item.id || !cyclicIds.has(item.id))) parent.children.push(item); else roots.push(item);
  }
  const sort = (arr: SessionSummary[]): void => { arr.sort((a,b) => (b.updated ?? b.mtime) - (a.updated ?? a.mtime) || a.fileName.localeCompare(b.fileName) || a.uriString.localeCompare(b.uriString)); arr.forEach((x) => sort(x.children)); };
