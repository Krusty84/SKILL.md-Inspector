...
const removedModuleNames = [
  'build' + 'Trace' + 'GraphViewModel',
  'build' + 'Session' + 'PathViewModel',
  'traceGraph' + 'State',
  'sessionPath' + 'State',
  removedBrowserBundle,
];
...
describe('OpenCode graph removal guards', () => {
  it('keeps the trace command out of commands, activation, menus, and welcomes', () => {
    expect(packageJson.contributes.commands.map((entry) => entry.command)).not.toContain(
      removedTraceCommand,
    );
  ...
  it.each([
    ...
    const item = new OpenCodeSessionTreeItem({
      uri,
  ...
  it('defines only the extension build output', () => {
    ...
    expect([...buildSource.matchAll(/outfile:\s*'([^']+)'/g)].map((match) => match[1])).toEqual([
      'dist/extension.js',
      'dist/webview/openCodeSessionReport.js',
    ]);
  });
