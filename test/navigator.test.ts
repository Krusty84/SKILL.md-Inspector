...
describe('navigator pure modules', () => {
  it('resolves built-in agent sources', () => {
    ...
    const defaultSources = resolveBuiltInAgentSources({ homeDir: '/h', env: {}, platform: 'win32' });
    expect(defaultSources).toContainEqual(expect.objectContaining({ id: 'codex-global', rootPath: path.join('/h', '.codex') }));
    expect(defaultSources).toContainEqual(expect.objectContaining({ id: 'codex-home-skills', rootPath: path.join('/h', '.codex', 'skills') }));
  ...
  it('normalizes additional roots and ignores malformed entries', () => {
    ...
    expect(result.roots).toEqual([
      { id: 'a', label: 'A', path: '~/.a', files: ['SKILL.md', 'AGENTS.md', 'CLAUDE.md'], recursive: true },
      { id: 'b', label: 'B', path: '/b', files: ['SKILL.md'], recursive: false },
      { id: 'c', label: 'C', path: '/c', files: ['CLAUDE.md'], recursive: false },
    ]);
  ...
  it('discovers external files within bounds', async () => {
    ...
    const result = await discoverExternalFiles({ id: 's', agentId: 'agent', agentLabel: 'Agent', groupLabel: 'Skills', rootPath: root, files: ['SKILL.md', 'AGENTS.md', 'CLAUDE.md'], recursive: true }, { maxResults: 10, maxDepth: 12 });
    expect(result.files.map((file) => file.relativePath)).toEqual(['a/b/AGENTS.md', 'CLAUDE.md', 'a/SKILL.md']);
    expect(result.files.find((file) => file.fileName === 'CLAUDE.md')).toEqual(expect.objectContaining({ fileName: 'CLAUDE.md', absolutePath: await realpath(path.join(root, 'CLAUDE.md')) }));
    const claude = await discoverExternalFiles({ id: 'claude', agentId: 'claude', agentLabel: 'Claude Code', groupLabel: 'Global Instructions', rootPath: root, files: ['CLAUDE.md'], recursive: false });
...
describe('navigator manifest', () => {
  it('declares containers, views, commands, menus, and additional roots schema', () => {
    expect(packageJson.contributes.viewsContainers.activitybar[0]).toMatchObject({ id: 'skillMdInspector', title: 'SKILL.md INSPECTOR', icon: 'media/skill-md-inspector.svg' });
    ...
    expect(packageJson.contributes.views.skillMdInspector.map((view) => view.id)).toEqual(['skillMdInspectorFavorites', 'skillMdInspectorWorkspace', 'skillMdInspectorInstalledAgents', 'skillMdInspectorOpenCodeSessions']);
    expect(packageJson.contributes.views.skillMdInspector.map((view) => view.name)).toEqual(['FAVORITES', 'WORKSPACE', 'INSTALLED AGENTS', 'OPENCODE SESSIONS']);
    ...
    expect(packageJson.contributes.configuration.properties['skillMdInspector.navigator.additionalRoots'].items.properties.files.items.enum).toContain('CLAUDE.md');
  });
});
