...
vi.mock('vscode', () => {
  class Uri {
    ...
    static parse(value: string): Uri {
      ...
      return new Uri(parsed.protocol.slice(0, -1), parsed.pathname);
    }
    static joinPath(parent: Uri, child: string): Uri {
      return new Uri(parent.scheme, `${parent.path.replace(/\/$/, '')}/${child}`);
    }
    toString(): string {
    ...
    workspace: {
      ...
      getConfiguration: vi.fn(() => ({
        get: vi.fn((_key: string, fallback: unknown) =>
          Object.keys(state.excludes).length ? state.excludes : fallback,
        ),
      })),
      getWorkspaceFolder: vi.fn((uri: any) =>
        state.folders.find((folder) => uri.toString().startsWith(folder.uri.toString())),
      ),
...
function folder(value: string, name: string, index: number): vscode.WorkspaceFolder {
  return { uri: uri(value), name, index };
}
...
describe('WorkspaceExplorer', () => {
  it('returns stable workspace roots in VS Code order', () => {
    ...
    expect(explorer.getRoots().map((root) => root.id)).toEqual([
      'workspace-root:mem:///a',
      'workspace-root:mem:///b',
    ]);
  });

  it('loads direct children lazily and caches expanded directories', async () => {
    state.folders = [folder('mem:///root', 'root', 0)];
    state.entries.set('mem:///root', [
      ['src', Directory],
      ['README.md', File],
      ['SKILL.md', File],
      ['AGENTS.md', File],
  ...
  it('matches standard exclude glob shapes used by files.exclude', () => {
    expect(
      matchesFilesExclude(uri('mem:///root'), uri('mem:///root/dist'), 'dist', '**/dist'),
    ).toBe(true);
    expect(
      matchesFilesExclude(uri('mem:///root'), uri('mem:///root/src/a.log'), 'a.log', '*.log'),
    ).toBe(true);
    expect(
      matchesFilesExclude(uri('mem:///root'), uri('mem:///root/src/a.ts'), 'a.ts', '*.log'),
    ).toBe(false);
  });
  ...
  it('sorts directories before files with case-insensitive numeric names', () => {
    ...
    expect(nodes.sort(compareWorkspaceNodes).map((node) => node.name)).toEqual([
      'folder2',
      'folder10',
      'file2.txt',
      'file10.txt',
